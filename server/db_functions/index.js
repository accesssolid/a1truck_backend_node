...
module.export = getJoinData = async (
  Model,
  query,
  feilds,
  lookup = null,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let data = await Model.aggregate()
        ...
        .limit(
          pagination !== null ? pagination.limit : Number.MAX_SAFE_INTEGER
        );
      if (data.length > 0) {
...
module.export = updateSingleData = (Model, filter, update, options) => {
  return new Promise((resolve, reject) => {
    Model.findOneAndUpdate(filter, update, options).exec((err, data) => {
      if (err || !data) {
        let response = helpers.showResponse(false, err);
        return resolve(response);
      }
      let response = helpers.showResponse(true, "data updated", data);
      return resolve(response);
    });
  });
};
